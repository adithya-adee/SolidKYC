...
export function IssueDOBCredentialCard({
    privateKey,
    onCredentialIssued,
}: IssueDOBCredentialCardProps) {
        ...
        try {
            ...
            if (
                ...
                toast.error("Credential Already Exists", {
                    description:
                        "A credential has already been issued to this wallet. Use a different wallet or wait for the existing credential to expire.",
                    duration: 6000,
                });
            } else if (error.message?.includes("credential")) {
                toast.error("Backend Error", {
                    description:
                        "Failed to issue credential. Make sure backend is running.",
                });
            } else {
                                ...
                                {connected && publicKey ? (
                                    ...
                                    <div className="p-3 bg-primary/10 rounded-lg flex items-center gap-2">
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            Wallet connection required to issue
                                            credential
                                        </p>
                                    </div>
                                )}
